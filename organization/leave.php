                                ...
                                <table class="table table-hover table-striped table-vcenter text-nowrap mb-0">
                                    ...
                                    while($leave=mysqli_fetch_assoc($res)){
                                        ...
                                        <tr>
                                            ...
                                            FROM employees where `id`=$leave[emp_id] ";
                                             $result=mysqli_query($conn,$query);
                                             while($row=mysqli_fetch_assoc($result)){
                                                echo '<td><div class="font-15">'.$row['name'].'</div></td>';
                                             }
                                             $start = new DateTime($leave['start']);
                                             $end = new DateTime($leave['end']);
                                             echo'                                        
                                            <td><span>'.$leave['emp_id'].'</span></td>
                                            <td>'.$start->format('d F, Y').' to '.$end->format('d F, Y').'</td>
                                            <td>'.$leave['reason'].'</td>
                                               <td>'.$leave['status'].'</td>
                                            <td>
                                             <a href="../backend/employee/leave.php?apr_id='.$leave['leave_id'].'" class="btn btn-icon btn-sm" title="Approved"><i class="fa fa-check text-success"></i></a>
                                              <a href="../backend/employee/leave.php?rej_id='.$leave['leave_id'].' " class="btn btn-icon" title="Reject"><i class="fa fa-ban text-danger"></i></a>
